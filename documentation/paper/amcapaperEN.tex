...
There is an increasing interest in using this type of models in a wide range of applications, such as in biological tissues modelling \citep{pellicer2020real}. In this work, a FEM solver  was repeatedly used to generate a large dataset of results, which was used to train the NN. The NN was designed to learn from the FEM solutions and predict the displacement field at particular nodes \citep{yang2022tracker}. Surrogate models can be used for structural assessment by tracking geometric features along time \citep{zhu2023visual} and for solving material identification problems \citep{steuben2015inverse}. Moreover existing methods such as Physics-Informed Neural Networks leverages on the underlying dynamics to train the model \citep{raissi2017physics}. 

In this work the methodology used to develop the NN, the performance results of the model compared to the FEM, and potential future directions are presented. By developing a faster surrogate model, this research line has the potential to significantly improve the efficiency of solving mechanical problems, with broader implications for the engineering design process.
...
\section{Conclusion}
In this work, a pipeline for generating a dataset of FEM solutions for simple solid mechanical problems was presented. The dataset was used to train a neural network to predict the displacements at control nodes. The results showed that the proposed approach was effective, with the neural network achieving low losses on both the training and test datasets, and closely matching the theoretical curve for uniaxial compression. It was also demonstrated that this approach has the potential to be extended for more complex mechanical problems, as shown by our evaluation on a cantilever model. 

One advantage of  this implementation is its simplicity and scalability. By using a relatively simple mechanical problem, it was possible to develop a pipeline that can be easily scaled to more complex problems. Additionally, the use of a neural network allows for faster computation times than the traditional FEM method. This has the potential to significantly reduce computational costs on any numerical processes that rely on FEM simulations, such as material identification algorithms for tissue disease diagnostics problems and stress CAD for manufacturing. However, as it is shown in the results, predictions for material properties outside the training space, may lead to higher error rates. 
